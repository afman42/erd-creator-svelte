...
export function edgePaths(schema) {
  ...
  for (const t of schema.tables)
    for (let i = 0; i < t.columns.length; i++) {
      ...
      let x1, x2;
      if (t.id === p.id) { x1 = t.x; x2 = t.x + 60; }
      else if (t.x > p.x) { x1 = p.x + BOX_W; x2 = t.x; }
      else if (t.x + BOX_W < p.x) { x1 = t.x + BOX_W; x2 = p.x; }
      else { x1 = t.x + BOX_W; x2 = p.x + BOX_W; }
      const mid = (x1 + x2) / 2;
      out.push({ d: `M ${x1} ${ci} C ${mid} ${ci}, ${mid} ${py}, ${x2} ${py}`, self: t.id === p.id });
    }
  return out;
}
