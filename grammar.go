...
// Dialect names. mariadb shares the mysql grammar (identical DDL, different
// header comment); sqlite is export-only and has no parser.
const (
	DialectMysql    = "mysql"
...
// mysql. Unknown values fall back rather than erroring: an unset dialect is the
// common case (every file written before this field existed).
...
// sqlite and mariadb are export-only: mariadb emits the mysql grammar with a
// different header, so it saves as mysql, but sqlite has no parser and would
// not survive a reopen.
func (s *Schema) saveable() bool {
	d := s.dialect()
	return d == DialectMysql || d == DialectPostgres
}
...
// changing it would churn all of them. The postgres header carries the suffix,
// which is what makes dialect detection on open possible.
// Output is pinned by TestGenSQLGolden / TestPostgresGolden.
...
func ParseDDL(sql string) (*Schema, error) {
	if detectDialect(sql) == DialectPostgres {
...
func detectDialect(sql string) string {
	for _, line := range strings.Split(sql, "\n") {
		...
		if strings.Contains(strings.ToLower(line), "postgres") {
			return DialectPostgres
		}
		if strings.Contains(strings.ToLower(line), "generated by erd-creator") {
...
// parseMysql reads the backtick/ENGINE=InnoDB grammar buildMysql emits.
func parseMysql(sql string) (*Schema, error) {
