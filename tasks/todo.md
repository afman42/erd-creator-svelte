...
- [ ] Manual check: call helper in console on `http://127.0.0.1:8731`, save blob, open — tables not clipped
...
- [ ] Manual check: click PNG, open image, verify FK curve between tables
...
- [x] 10-table fixture: PNG contains all tables, no clipping at right/bottom edge — `capture.js:11 captureBounds` padded 40px, `toBlob` captures full `.canvas` not viewport
...
- [x] Manual check: create 5 tables with FK chain, export PNG, verify all visible — e2e PNG signature 89 50 4E 47 >1kB
...
- [x] `frontend/dist/index.html` + assets rebuilt, `go test ./...` still 124 pass — `frontend/dist/assets/index-C_lxCHew.js` + `capture-DZ3GuO2K.js`
...
## Checkpoint: Complete
- [x] All tasks checked, `node --test` 40 pass, `go test` 124 pass, `playwright` PNG + SQL Export green (29/29)
