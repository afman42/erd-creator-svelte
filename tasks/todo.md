...
- [ ] T1: Toolchain spike — build svelte fork + scaffold app (S)
...
- [ ] T2: Model store + immutable mutations (M)
...
- [ ] App boots; model layer green standalone.

## Phase 1: Core vertical slice — "draw one table"

- [ ] T3: Entity + field editor panels (M)
...
- [ ] T4: Canvas render + click-drag + arrow-key move (M)
...
- [ ] Manual: create 2 positioned entities end-to-end.

- [ ] T5: Relationship editor + orthogonal edge overlay (L→split if slips: 5a editor, 5b routing)
...
- [ ] T8: Undo/redo + help overlay + mouse drag (M)
  ...
  - Depends: T3–T7.
- [ ] Full flow: new → 3 entities → fields → 2 relationships → save → load → export SQL. Tests green. `pnpm run build` ships single file.
