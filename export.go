...
func mysqlColDef(c Col) string {
	...
	// DEFAULT is emitted as raw (validated) text, like the type: the value is
	// open-ended (CURRENT_TIMESTAMP, '(uuid())', 'it''s', 0), so it cannot be
	// quoted as one token. Validation (validateDefault) and the validateOutput
	// last gate are what keep it from becoming injection. Empty means "no
	// clause", which is what keeps files written before the field existed
	// byte-identical.
	if c.Default != "" {
		s += " DEFAULT " + c.Default
	}
...
func buildMysql(tables []Table, header string) string {
	...
	for _, t := range tables {
		...
		// table comment at all (buildSqlite omits it — documented lossy), and
		// Postgres uses a separate COMMENT ON TABLE statement (buildPostgres).
		close := ") ENGINE=InnoDB"
		if t.Comment != "" {
			// sqlStr already wraps in quotes and doubles embedded ' — a table
			// comment is a string literal, not raw text.
			close += " COMMENT=" + sqlStr(t.Comment)
		}
...
func postgresColDef(c Col) string {
	...
	s := quoteDQ(c.Name) + " " + ty
	// DEFAULT sits before NOT NULL, matching Postgres' canonical column
	// layout. It is emitted raw like the mysql path (validated first); the
	// AI/IDENTITY + DEFAULT combination is refused by validateShape, so the
	// two clauses never appear together.
	if c.Default != "" {
		s += " DEFAULT " + c.Default
	}
	if c.Nn || c.Pk {
...
func buildPostgres(tables []Table) string {
	...
	for _, t := range tables {
		...
		// Table comment first, so the out-of-line block reads table-level
		// facts before the columns they describe.
		if t.Comment != "" {
			post = append(post, fmt.Sprintf("COMMENT ON TABLE %s IS %s;",
				quoteDQ(t.Name), sqlStr(t.Comment)))
		}
		for _, c := range t.Columns {
